...
const float g_EyeCenterX = 0.f;
const float g_EyeCenterY = 10.f;
const float g_EyeCenterZ = 30.f;
float g_EyeRadius = 8.f, g_EyeRotate = 0.f;
float g_EyeX = g_EyeCenterX, g_EyeY = g_EyeCenterY, g_EyeZ = g_EyeCenterZ;
...
void display() {
	...
	// 回転軸は柱を中心として時計回り
    glTranslatef(0, -1.f, 0);
    glRotatef(g_RotationDegree, 0, 1.f, 0);  
	...
	for (int i = 0; i < g_NumTeapots; i++) {
		...
		// ティーポットの高さ方向の値
		// g_RotationDegreeを利用
		// sinの正値にして、台の厚み分を底上げする
		const float yPos = sin(thetaDegree + g_RotationDegree * 0.3) + 1.f + 0.7f;
...
void timer(int val) {
	...
	// ズームしたり、引いたりする
	g_EyeRadius -= 0.05f;
	g_EyeX = g_EyeCenterX + g_EyeRadius * cos(rotationRad);
	g_EyeY += 0.01;
	g_EyeZ = g_EyeCenterZ + g_EyeRadius * sin(rotationRad);

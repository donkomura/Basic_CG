...
void updateCloth(void) {
	...
	for(int i = 0; i < cloth->springs.size(); i++) {// 全てのバネについて順番に処理する
		...
		Vector3d _p0 = spring->p0->p;
		Vector3d _p1 = spring->p1->p;
		double _l = sqrt((_p0-_p1)*(_p0-_p1));
		
		// (b) バネが質点に加える力の大きさを求める
		// （自然長 - 現在の長さ）にバネ定数 Ks を掛けた値が求める大きさ
		double _f = Ks * (spring->restLength - _l);
		
		// (c) バネが質点に加える力の向き（単位ベクトル）を求める
		// バネには、両端に質点がついているので、一方から他方に向かう方向が力の向き
		Vector3d e0 = spring->p0->p - spring->p1->p;
		Vector3d e1 = spring->p1->p - spring->p0->p;
		e0.normalize();
		e1.normalize();

...
void setCoordinates() {
	...
	for(int i = 0; i < NUM_U+1; i++) {
		for(int j = 0; j < NUM_V+1; j++) {
			...
			x[i][j] = u;
			y[i][j] = v;
			z[i][j] = 0.1 * sin(8.0 * PI * sqrt((u-0.5) * (u-0.5) + (v-0.5) * (v-0.5)));
		}
	}
}
	...
	*/
	...
	for(int i = 0; i < NUM_U; i++) {
		for(int j = 0; j < NUM_V; j++) {
			...
			// OBJ形式では頂点番号は1から始まる
			int lb_index = (NUM_V + 1) * j + i + 1; // 左下の頂点番号を i,j,NUM_U,NUM_Vの値から計算する
			int lt_index = lb_index + 1; // 左上の頂点番号の計算式
			int rb_index = lb_index + (NUM_V + 1); // 右下の頂点番号の計算式
			int rt_index = rb_index + 1; // 右上の頂点番号の計算式

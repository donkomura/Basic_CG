	...
	if(t_board > 0) { // 床との交点がある
		...
		for (double xi = 0.0; xi < 1.0; xi += 1.0 / 3.0) {
			for (double yi = 0.0; yi < 1.0; yi += 1.0 / 3.0) {
                ...
                Vector3d _to_board = viewPosition + _t_board * _ray;
                double _r, _g, _b;
                if (_to_board.z < -3000) {
					continue;
                } else {
                    ...
                    double Id = (norm * (-lightDirection)) < 0 ? 0 : Iin * Kd * (norm * -lightDirection); // 拡散反射光
                    double I = Ia + Id;
                    ...
                    Vector3d light = -lightDirection;
                    double t_board_shadow = sphere.getIntersec(_to_board, light);
                    if (t_board_shadow > 0) {
